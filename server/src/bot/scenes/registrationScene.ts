...
export class RegistrationScene {
    ...
    createScene() {
        ...
        registrationScene.on('contact', async (ctx) => {
            ...
            if (this.validationService.validatePhoneNumber(ctx.message.contact.phone_number)) {
                ctx.session.phone = ctx.message.contact.phone_number;
                const replyMessage = await ctx.reply('Чудово! Тепер надішліть свою електронну пошту.', Markup.removeKeyboard());
                if ('message_id' in replyMessage) {
                    ctx.session.messageIds.push(replyMessage.message_id);
                }
                ctx.session.step = 2;
            } else {
                const replyMessage = await ctx.reply('Введений номер телефону здається неправильним. Будь ласка, спробуйте ще раз.');
                if ('message_id' in replyMessage) {
                    ctx.session.messageIds.push(replyMessage.message_id);
                }
            }
            ...
            switch (ctx.session.step) {
                case 1:
                    if (this.validationService.validatePhoneNumber(ctx.message.text)) {
                        ...
                        const replyMessage = await ctx.reply('Чудово! Тепер надішліть свою електронну пошту.');
                        if ('message_id' in replyMessage) {
                            ctx.session.messageIds.push(replyMessage.message_id);
                        }
                        ctx.session.step = 2;
                ...
                case 2:
                    if (this.validationService.validateEmail(ctx.message.text)) {
                        const existingUser = await this.usersService.findByEmail(ctx.message.text);
                        if (existingUser) {
                            const replyMessage = await ctx.reply('Ця електронна адреса вже використовується. Будь ласка, введіть іншу.');
                            if ('message_id' in replyMessage) {
                                ctx.session.messageIds.push(replyMessage.message_id);
                            }
                            return;
                        ...
                        await this.emailService.sendVerificationEmail(ctx.session.email, ctx.from.first_name, verificationCode);
                        ...
                        const replyMessage = await ctx.reply('Введена адреса електронної пошти неправильна. Будь ласка, спробуйте ще раз.');
                        if ('message_id' in replyMessage) {
                            ctx.session.messageIds.push(replyMessage.message_id);
                        }
                    }
                ...
                case 3:
                    if (ctx.message.text === ctx.session.verificationCode) {
                        const replyMessage = await ctx.reply('Код підтвердження правильний! Тепер, будь ласка, створіть надійний пароль.');
                        if ('message_id' in replyMessage) {
                            ctx.session.messageIds.push(replyMessage.message_id);
                        }
                        ctx.session.step = 4;
                ...
                case 4:
                    if (this.validationService.validatePassword(ctx.message.text)) {
                        ...
                        const name = (firstName + " " + lastName).trim() || 'No name provided';
                        ...
                        const sentMessage = await ctx.reply(`Реєстрацію завершено! Вітаємо, ${newUser.name}. Тепер ти можеш використовувати всі функції боту. Краще видали повідомлення з паролем із чату, для безпеки твого акаунту!`);
                        setTimeout(() => {
                            this.deleteMessageService.deleteMessage(ctx.chat.id, sentMessage.message_id);
                        }, 30000);

                        for (const messageId of ctx.session.messageIds) {
                            await this.deleteMessageService.deleteMessage(ctx.chat.id, messageId);
                        }
                        ...
                        if (user) {
                            await ctx.reply('Вітаємо у нашому GameBuzz Marketplace! Як я можу допомогти вам сьогодні?',
                                this.menuService.createMainMenu());
                        } else {
                            ctx.scene.enter('registration');
                        }
                    } else {
                        const replyMessage = await ctx.reply('Пароль не відповідає вимогам. Мінімум 8 символів. Спробуйте ще раз.');
                        if ('message_id' in replyMessage) {
                            ctx.session.messageIds.push(replyMessage.message_id);
                        }
                    }
        ...
        registrationScene.action('resend_code', async (ctx) => {
            ...
            if (currentTime - ctx.session.lastResendTime < 120000) {  // 120000ms = 2 minutes
                await ctx.answerCbQuery('Ми можемо відправити код ще раз тільки через 2 хвилини.');
                return;
            }
            const verificationCode = Math.floor(100000 + Math.random() * 900000).toString();
            await this.emailService.sendVerificationEmail(ctx.session.email, ctx.from.first_name, verificationCode);
