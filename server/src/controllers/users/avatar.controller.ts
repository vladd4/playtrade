...
export class AvatarController {
  ...
  @ApiResponse({
    status: 500,
    description: 'Ошибка сервера при загрузке аватарки',
  })
  ...
  @ApiBody({
    ...
    examples: {
      uploadAvatarExample: {
        ...
        description:
          'Пример multipart запроса для загрузки аватарки пользователя',
        value: {
          file: 'file',
        },
      },
  ...
  async uploadAvatar(
    ...
    try {
      ...
      const updatedUser = await this.usersService.update(id, {
        avatarPhoto: `/api/uploads/avatars/${file.filename}`,
      });
      return updatedUser;
    } catch (e) {
