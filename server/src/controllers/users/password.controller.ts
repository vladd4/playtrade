...
export class PasswordController {
  ...
  @ApiResponse({
    status: 404,
    description: 'Пользователь не найден или email не совпадает',
  })
  @ApiResponse({
    status: 500,
  ...
  async requestPasswordReset(
    ...
    try {
      ...
      if (user.email !== email) {
        return res
          .status(HttpStatus.NOT_FOUND)
          .json({ message: 'Email не збігається із зареєстрованим' });
      }
      ...
      return res
        ...
        .json({ message: 'Тимчасовий пароль було надіслано на ваш email' });
    } catch (e) {
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: 'Помилка під час скидання пароля' });
    }
  }
    ...
    try {
      ...
      if (!isMatch) {
        return res
          ...
          .json({ message: 'Неверный текущий пароль' });
      }
      ...
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: 'Ошибка при обновлении пароля' });
    }
  }
}
