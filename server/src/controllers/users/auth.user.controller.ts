...
export class AuthUserController {
    ...
    async login(@Body() loginDto: LoginDto, @Req() req: Request): Promise<any> {
        try {
            ...
            if (!user) {
                throw new UnauthorizedException('Неправильні облікові дані. Помилка у телеграм айді');
            }
            ...
            const isMatch = await bcrypt.compare(loginDto.password, base64.decode(user.password));
            if (!isMatch) {
                throw new UnauthorizedException('Неправильні облікові дані. Помилка з паролем');
            }
    ...
    @ApiResponse({ status: 500, description: 'Ошибка сервера при обновлении сессии' })
