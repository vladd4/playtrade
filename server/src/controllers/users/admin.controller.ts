...
export class AdminController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Список администраторов и менеджеров с пагинацией',
  })
  async getAdminsAndManagers(
    ...
    const { users, totalCount } =
      await this.usersService.findAllAdminsAndManagers(limit, page);
  ...
  @Get('/check-ban/admin')
  @ApiOperation({
    summary: 'Проверка статуса бана пользователя по Telegram ID',
  })
  @ApiQuery({
    name: 'telegramId',
    required: true,
  ...
  @ApiResponse({
    status: 200,
    description: 'Статус бана проверен',
    type: Boolean,
  })
  @ApiResponse({ status: 404, description: 'Пользователь не найден' })
  ...
  async checkUserBanStatus(
    ...
    try {
      const isBanned =
        await this.usersService.isUserBannedByTelegramId(telegramId);
      return { isBanned };
    } catch (error) {
  ...
  async searchByName(@Query('name') name: string): Promise<UserEntity[]> {
    try {
      if (!name) {
  ...
  async updateUserBalance(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Помилка при оновленні балансу користувача',
      );
    }
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Статус верифікації користувача успішно оновлено',
  })
  @ApiResponse({ status: 404, description: 'Користувача не знайдено' })
  @ApiResponse({
    status: 400,
    description: 'Невірне значення статусу верифікації',
  })
  @ApiResponse({
    status: 500,
    description: 'Помилка при оновленні статусу верифікації користувача',
  })
  async updateUserVerificationStatus(
    ...
    try {
      ...
      return {
        message: 'Статус верифікації користувача успішно оновлено',
        user,
      };
    } catch (e) {
      if (e instanceof NotFoundException) {
      ...
      throw new InternalServerErrorException(
        'Помилка при оновленні статусу верифікації користувача',
      );
    }
  }
}
