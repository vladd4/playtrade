...
export class UsersController {
    ...
    @ApiQuery({ name: 'limit', required: false, description: 'Кількість користувачів на сторінку', type: Number })
    @ApiQuery({ name: 'page', required: false, description: 'Номер сторінки', type: Number })
    @ApiResponse({ status: 200, description: 'Список користувачів успішно отримано', type: [UserEntity] })
    @ApiResponse({ status: 500, description: 'Помилка сервера при отриманні списку користувачів' })
    async findAll(
        @Query('limit') limit: number = 10,
        @Query('page') page: number = 1
    ): Promise<any> {
        const { users, totalCount } = await this.usersService.findAllWithPagination(limit, page);
    ...
    @ApiResponse({ status: 200, description: 'Пользователь успешно найден', type: UserEntity })
    @ApiResponse({ status: 404, description: 'Пользователь не найден' })
    @ApiResponse({ status: 500, description: 'Ошибка сервера при поиске пользователя' })
    ...
    @ApiResponse({ status: 200, description: 'Имя пользователя успешно обновлено' })
    @ApiResponse({ status: 404, description: 'Пользователь не найден' })
    @ApiResponse({ status: 500, description: 'Ошибка при изменении имени пользователя' })
    @ApiParam({ name: 'id', description: 'ID пользователя', type: 'string' })
    async updateUserName(
        ...
        try {
            ...
            const updatedUser = await this.usersService.update(id, { name: body.newName });

            return { message: 'Имя пользователя успешно обновлено', user: classToPlain(updatedUser) };
        } catch (e) {
            throw new InternalServerErrorException('Ошибка при изменении имени пользователя');
        }
    }
    ...
    @ApiResponse({ status: 404, description: 'Пользователь не найден' })
    @ApiResponse({ status: 500, description: 'Ошибка сервера при поиске продуктов' })
    @ApiParam({ name: 'id', description: 'ID пользователя', type: 'string' })
    ...
    @ApiResponse({ status: 404, description: 'Пользователь не найден' })
    @ApiResponse({ status: 500, description: 'Ошибка сервера при поиске покупок' })
    @ApiParam({ name: 'id', description: 'ID пользователя', type: 'string' })
