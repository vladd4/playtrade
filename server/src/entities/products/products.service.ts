...
export class ProductsService {
  ...
  async findOneWithIds(
    ...
    Partial<ProductDto & { reviewCount: number; latestReviews: ReviewDto[] }>
  > {
    ...
    // const reviewCount = await this.reviewsService.getReviewCountForUser(
    //   product.owner.id,
    // );
    // const latestReviews = await this.reviewService.getLatestReviewsForUser(
    //   product.owner.id,
    // );
  ...
  async update(
    ...
    if (images && images.length > 0) {
      ...
      const totalImages =
        (product.imageUrls || []).length + newImageUrls.length;
  ...
  async findByTypeAndGameMiniWithPagination(
    type: ProductType,
    gameId: string,
    limit: number,
    page: number,
  ...
  async findByTypeAndGameWithFilters(
    ...
    try {
      const queryBuilder = this.productRepository
        ...
        .andWhere('product.inProcess = :inProcess', { inProcess: false });

      if (platforms && platforms.length > 0) {
        if (typeof platforms === 'string') {
          platforms = [platforms];
        }
        queryBuilder.andWhere('product.platform IN (:...platforms)', {
          platforms,
        });
      }
