...
export class ProductsController {
  ...
  @ApiOperation({
    summary:
      'Получение списка всех продуктов с пагинацией и общим количеством страниц',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 500,
    description: 'Ошибка при получении списка продуктов',
  })
  async findAll(
  ...
  @ApiResponse({
    status: 200,
    description: 'Список ID продуктов успешно получен',
  })
  @ApiResponse({
    status: 500,
    description: 'Ошибка при получении списка ID продуктов',
  })
  async findAllIds(): Promise<any> {
    try {
      ...
      throw new InternalServerErrorException(
        'Ошибка при получении списка ID продуктов',
      );
    }
  }
  ...
  @Get(':id')
  @ApiOperation({
    summary: 'Получение информации о продукте по ID с ID игры и владельца',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 500,
    description: 'Ошибка при получении информации о продукте',
  })
  ...
  @ApiOperation({
    summary: 'Получение продуктов по типу и ID игры с фильтрацией',
  })
  @ApiResponse({
    status: 200,
  ...
  async findByTypeAndGameAndFiltration(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Ошибка при получении продуктов по типу и ID игры',
      );
    }
  }
  @Get('admin/search')
  ...
  async searchByName(@Query('name') name: string): Promise<ProductDto[]> {
    try {
      if (!name) {
    ...
    FilesInterceptor('images', 10, {
      ...
      fileFilter: (req, file, cb) => {
        if (!file.mimetype.match(/\/(jpg|jpeg|png)$/)) {
          return cb(
            new Error(
              'Разрешены только файлы изображений формата JPG, JPEG, PNG!',
            ),
            false,
          );
        }
  ...
  @ApiOperation({
    summary: 'Создание нового продукта с фотографиями, ID игры и ID владельца',
  })
  @ApiResponse({
    status: 201,
  ...
  async create(
    ...
    try {
      if (images && images.length > 0) {
        productDto.imageUrls = images.map(
  ...
  async update(
    ...
    try {
      ...
      const currentImages = existingProduct.imageUrls
        ? existingProduct.imageUrls.length
        : 0;
      const newImages = images.length;

      if (currentImages + newImages > 5) {
        throw new BadRequestException(
          'Нельзя добавить больше 5 изображений к продукту',
        );
      }
      ...
      if (body.detailDescription)
        updateData.detailDescription = body.detailDescription;
  ...
  @ApiResponse({
    status: 500,
    description: 'Ошибка при изменении статуса активности',
  })
