...
export class AdminCommentController {
    ...
    @ApiResponse({ status: 201, description: 'Комментарий успешно добавлен', type: AdminComment })
    @ApiBody({
    ...
    @Get()
    @ApiOperation({ summary: 'Получение всех комментариев администратора для чата' })
    @ApiResponse({ status: 200, description: 'Список комментариев успешно получен', type: [AdminComment] })
    async getCommentsByChat(@Param('chatId') chatId: string): Promise<AdminComment[]> {
