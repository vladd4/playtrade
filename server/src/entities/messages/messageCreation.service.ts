...
export class MessageCreationService {
    ...
    async create(createMessageDto: CreateMessageDto): Promise<MessageEntity> {
        ...
        if (createMessageDto.productId) {
            product = await this.productRepository.findOne({ where: { id: createMessageDto.productId } });
            if (!product) {
        ...
        const chat = await this.chatValidationService.validateChatExists(createMessageDto.chatId);
        if (!chat) {
    ...
    async createSupportMessage(createSupportMessageDTO: CreateSupportMessageDTO): Promise<MessageEntity> {
