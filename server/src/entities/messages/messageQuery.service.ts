...
export class MessageQueryService {
    ...
    async findMessagesByUserAndChat(userId: string, chatId: string, type: 'sent' | 'received'): Promise<MessageEntity[]> {
        console.log(type)
        console.log(chatId)
        console.log(userId)
    ...
    async findMessagesByUser(userId: string, type: 'sent' | 'received'): Promise<any[]> {
        const queryBuilder = this.messageRepository.createQueryBuilder('message')
            .leftJoinAndSelect('message.receiver', 'receiver')
            .leftJoinAndSelect('message.product', 'product')
            .leftJoinAndSelect('message.chat', 'chat')
    ...
    async findMessagesByUserAndProduct(buyerId: string): Promise<MessageEntity[]> {
        return this.messageRepository.find({
            where: {
                sender: { id: buyerId },
            },
    ...
    async markMessagesAsRead(chatId: string, userId: string): Promise<void> {
        this.logger.log(`Attempting to mark messages as read for chatId: ${chatId} and userId: ${userId}`);

        const chatExists = await this.messageRepository.createQueryBuilder('message')
            .where('message.chat_id = :chatId', { chatId })
            .andWhere('(message.senderId = :userId OR message.receiverId = :userId)', { userId })
            .getOne();

        if (!chatExists) {
            throw new NotFoundException(`Chat with ID ${chatId} between user ${userId} not found`);
        }
