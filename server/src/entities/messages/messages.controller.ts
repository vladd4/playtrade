...
export class MessagesController {
    ...
    @Post()
    async create(@Body() createMessageDto: CreateMessageDto, @Res() res: Response): Promise<Response> {
        try {
            const message = await this.messagesCreationService.create(createMessageDto);
            ...
            return res.status(HttpStatus.BAD_REQUEST).json({ message: 'Error creating message', error: error.message });
        }
    }
    ...
    @ApiResponse({ status: 200, description: 'Сообщения успешно получены', type: [MessageEntity] })
    ...
    async findAll(@Res() res: Response): Promise<Response> {
        try {
            ...
            return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error retrieving messages', error: error.message });
        }
    }

    @Get('id/:id')
    @ApiOperation({ summary: 'Получение сообщения по ID' })
    @ApiParam({ name: 'id', description: 'UUID сообщения', example: 'c9b1a16e-5f2f-46b3-b8e4-79f4162b2c56' })
    @ApiResponse({ status: 200, description: 'Сообщение успешно найдено', type: MessageEntity })
    ...
    async findOne(@Param('id') id: string, @Res() res: Response): Promise<Response> {
        try {
            ...
            return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error retrieving the message', error: error.message });
        }
    }
    ...
    @Get('by-chat')
    @ApiOperation({ summary: 'Получение сообщений пользователя в определенном чате' })
    @ApiQuery({ name: 'userId', description: 'UUID пользователя', required: true })
    @ApiQuery({ name: 'chatId', description: 'UUID чата', required: true })
    @ApiQuery({ name: 'type', description: 'Тип сообщений: отправленные (sent) или полученные (received)', required: true })
    @ApiResponse({ status: 200, description: 'Сообщения пользователя успешно найдены', type: [MessageEntity] })
    @ApiResponse({ status: 500, description: 'Ошибка при получении сообщений' })
    async findMessages(
        @Query('userId') userId: string,
        @Query('chatId') chatId: string,
        ...
        try {
            const messages = await this.messagesQueryService.findMessagesByUserAndChat(userId, chatId, type);
            return res.status(HttpStatus.OK).json(messages);
        } catch (error) {
            return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error retrieving messages', error: error.message });
        }
    ...
    async delete(@Param('id') id: string, @Res() res: Response): Promise<Response> {
        try {
            ...
            return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error deleting message', error: error.message });
        }
    }
}
