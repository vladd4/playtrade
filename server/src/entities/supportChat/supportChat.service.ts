...
export class SupportChatService {
    ...
    async createOrUpdateChatWithLastMessage(userId: string, content: string, receiverId: string): Promise<MessageEntity> {
        ...
        if (chat && chat.isClosed) {
            ...
            console.log(`Используется существующий активный чат для пользователя с ID ${userId}.`);
        }
        ...
        console.log(`Сообщение отправлено в чат с ID ${chat.id}. Сообщение: ${JSON.stringify(message)}`);
    ...
    async findAllWithPagination(limit: number, page: number): Promise<{ chats: SupportChat[], totalCount: number }> {
    ...
    async getChatById(chatId: string): Promise<SupportChat> {
        const chat = await this.supportChatRepository.findOne({
            where: { id: chatId },
            relations: ['user', 'messages', 'messages.sender', 'messages.receiver','lastMessage', 'lastMessage.sender', 'lastMessage.receiver'],
        });
        ...
        const chatWithUserIdAndSenderId = {
            ...
            lastMessage: chat.lastMessage
                ? {
                    ...chat.lastMessage,
                    senderId: chat.lastMessage.sender ? chat.lastMessage.sender.id : null,
                    sender: undefined,
                    receiverId: chat.lastMessage.receiver? chat.lastMessage.receiver.id : null,
                    receiver: undefined
                }
                : null,
        };
