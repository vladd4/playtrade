...
export class SupportChatController {
  ...
  @ApiOperation({
    summary:
      'Получение всех чатов техподдержки с пагинацией и общим количеством страниц',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  async getAllChats(
    ...
    const { chats, totalCount } =
      await this.supportChatService.findAllWithPagination(limit, page);
  ...
  @ApiParam({
    name: 'chatId',
    description: 'ID чата техподдержки',
    type: 'string',
  })
  @ApiResponse({
    status: 200,
  ...
  async uploadImage(
    ...
    try {
      const result = await this.supportChatPhotoService.processPhoto(
        file,
        senderId,
        adminId,
      );
