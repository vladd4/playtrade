...
export class GamesService {
    ...
    async updateGame(id: string, gameDto: GameDto, photo?: MulterFile): Promise<Game> {
    ...
    async addPlatformsToGame(gameId: string, newPlatforms: string[]): Promise<Game> {
        const game = await this.gameRepository.findOne({ where: { id: gameId } });
        ...
        game.platforms = Array.from(new Set([...(game.platforms || []), ...newPlatforms]));
    ...
    async addServersToGame(gameId: string, newServers: string[]): Promise<Game> {
        ...
        game.servers = Array.from(new Set([...(game.servers || []), ...newServers]));
    ...
    async removePlatform(gameId: string, platform: string): Promise<Game> {
        ...
        game.platforms = game.platforms.filter(existingPlatform => existingPlatform !== platform);
    ...
    async removeServer(gameId: string, server: string): Promise<Game> {
        ...
        game.servers = game.servers.filter(existingServer => existingServer !== server);
    ...
    async removeRegion(gameId: string, region: string): Promise<Game> {
        ...
        game.region = game.region.filter(existingRegion => existingRegion !== region);
