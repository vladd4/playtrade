...
export class TransactionsController {
  ...
  @ApiOperation({
    summary:
      'Получение списка всех транзакций с пагинацией и общим количеством страниц',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 500,
    description: 'Ошибка при получении списка транзакций',
  })
  async findAll(
    @Res() res: Response,
    ...
    try {
      const { transactions, totalCount } =
        await this.transactionsService.findAllTransactions(limit, page);
      ...
      return res
        ...
        .json({ message: 'Ошибка при получении списка транзакций' });
    }
  }
