...
export class TransactionsService {
    ...
    async findAllTransactions(limit: number, page: number): Promise<{ transactions: TransactionEntity[], totalCount: number }> {
    ...
    async findTransactionById(id: string): Promise<TransactionEntity> {
        const transaction = await this.transactionRepository.findOne({ where: { id }, relations: ['sender', 'receiver'] });
        if (!transaction) {
