...
export class ReviewsController {
  ...
  async findAllBySeller(
    ...
    try {
      ...
      return res
        ...
        .json({ message: 'Error retrieving reviews by seller ID' });
    }
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Последние отзывы успешно получены',
    type: [ReviewDto],
  })
  async findLatest(@Res() res: Response): Promise<Response> {
    try {
      ...
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: 'Error retrieving latest reviews' });
    }
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Количество отзывов успешно получено',
  })
  async countReviewsLastYear(@Res() res: Response): Promise<Response> {
    try {
      ...
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: 'Error retrieving review count for last year' });
    }
  }
  @Get()
  ...
  async findOne(
    ...
    try {
      ...
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: 'Error retrieving the review', error: error.message });
    }
  }
  ...
  async remove(
    ...
    try {
      ...
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: 'Error deleting the review', error: error.message });
    }
  }
}
