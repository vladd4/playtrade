...
export class UserEntity {
    @ApiProperty({ example: 'c9b1a16e-5f2f-46b3-b8e4-79f4162b2c56', description: 'Уникальный идентификатор пользователя' })
    @PrimaryGeneratedColumn('uuid')
    id: string;

    @ApiProperty({ example: 'http://example.com/avatar.jpg', description: 'URL фотографии аватара пользователя', nullable: true })
    ...
    @ApiProperty({ example: '+1234567890', description: 'Номер телефона пользователя', nullable: true })
    @Column({ nullable: true, default: null })
    phoneNumber: string;

    @ApiProperty({ example: 'example@example.com', description: 'Email пользователя', nullable: true })
    @Column({ nullable: true, default: null })
    email: string;

    @ApiProperty({ example: 'johndoe', description: 'Имя пользователя в Telegram', nullable: true })
    @Column({ nullable: true, default: null })
    ...
    @ApiProperty({ example: 100, description: 'Баланс пользователя', nullable: true })
    @Column({ nullable: true, default: null })
    balance: number;

    @ApiProperty({ example: 4.5, description: 'Рейтинг пользователя', nullable: true })
    ...
    @ApiProperty({ description: 'Идентификатор сессии пользователя', nullable: true })
    @Column({ nullable: true })
    sessionId: string;

    @ApiProperty({ example: '2023-12-31T23:59:59Z', description: 'Дата створення користувача', nullable: false })
    @CreateDateColumn({ type: 'timestamp' })
    createdAt: Date;

    @ApiProperty({ example: '2023-12-31T23:59:59Z', description: 'Дата, до которой пользователь забанен', nullable: true })
    ...
    @Exclude()
    @OneToMany(() => MessageEntity, message => message.receiver, { eager: false })
    messagesReceived: MessageEntity[];

    @Exclude()
    @OneToMany(() => TransactionEntity, transaction => transaction.sender, { eager: false })
    sentTransactions: TransactionEntity[];

    @Exclude()
    @OneToMany(() => TransactionEntity, transaction => transaction.receiver, { eager: false })
