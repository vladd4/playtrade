...
export class UsersService {
    ...
    async update(userId: string, updateData: Partial<UserEntity>): Promise<UserEntity> {
    ...
    async findAllWithPagination(limit: number, page: number): Promise<{ users: UserEntity[], totalCount: number }> {
    ...
    async banUserPermanently(userId: string): Promise<void> {
        await this.userRepository.update(userId, { isBanned: true, banUntil: null });
    }

    async unbanUser(userId: string): Promise<UserEntity> {
        await this.userRepository.update(userId, { isBanned: false, banUntil: null });
        return this.findOne(userId);
    }
    ...
    async updateVerificationStatus(userId: string, isVerified: boolean): Promise<UserEntity> {
        const user = await this.findOne(userId);
        if (!user) {
            throw new NotFoundException('Користувача не знайдено');
        }
    ...
    async findAllAdminsAndManagers(limit: number, page: number): Promise<{ users: UserEntity[], totalCount: number }> {
        const skip = (page - 1) * limit;
        const [users, totalCount] = await this.userRepository.findAndCount({
            where: [
                { role: UserRole.ADMIN },
    ...
    formatUserResponse(user: UserEntity): any {
        return {
            ...
            transactions: user.receivedTransactions?.map(transaction => ({
                product: transaction.sender ? transaction.sender.name : 'Невідомий товар',
                id: transaction.id,
                date: transaction.createdAt,
                amount: `${transaction.amount} GB coins`,
                status: transaction.status === 'completed' ? 'успішно' : transaction.status,
            })) || [],
        };
    }
