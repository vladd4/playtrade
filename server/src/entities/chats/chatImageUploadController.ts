...
import { ApiOperation, ApiResponse, ApiConsumes, ApiTags, ApiBody, ApiParam } from '@nestjs/swagger';
...
export class ChatImageUploadController {
    ...
    @ApiResponse({ status: 201, description: 'Изображение успешно загружено и возвращен URL', schema: { example: { imageUrl: 'http://localhost:3001/uploads/chat-images/unique-image.jpg' } } })
    ...
    @ApiParam({ name: 'imageName', description: 'Имя файла изображения для удаления', example: 'unique-image.jpg' })
    ...
    async deleteImage(@Param('imageName') imageName: string) {
        const imagePath = path.join(process.cwd(), 'api/uploads/chat-images', imageName);
