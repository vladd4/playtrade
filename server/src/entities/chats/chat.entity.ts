...
export class Chat {
    ...
    @ApiProperty({ example: 'true, false', description: 'Чат в избранном или нет' })
    ...
    @ApiProperty({ type: () => Product, description: 'Продукт, связанный с чатом' })
    @ManyToOne(() => Product, { nullable: true })
    product: ChatProductDto;

    @ApiProperty({ type: () => [AdminComment], description: 'Комментарии администратора, связанные с чатом' })
    @OneToMany(() => AdminComment, comment => comment.chat)
    adminComments: AdminComment[];

    @ApiProperty({ example: '2023-01-01T00:00:00Z', description: 'Дата создания чата' })
    @CreateDateColumn()
    createdAt: Date;
}
