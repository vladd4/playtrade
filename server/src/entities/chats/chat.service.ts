...
export class ChatsService {
    ...
    async createChat(owner: UserEntity, receiver: UserEntity, product: Product): Promise<Chat> {
        const chat = this.chatsRepository.create({ owner, participants: [owner, receiver], product });
        return this.chatsRepository.save(chat);
    }
    async findChatBetweenUsers(ownerId: string, receiverId: string, productId: string): Promise<Chat | null> {
        return this.chatsRepository.createQueryBuilder('chat')
            .innerJoinAndSelect('chat.participants', 'participants')
            .where('chat.ownerId = :ownerId AND participants.id = :receiverId AND chat.productId = :productId', { ownerId, receiverId, productId })
            .orWhere('chat.ownerId = :receiverId AND participants.id = :ownerId AND chat.productId = :productId', { ownerId: receiverId, receiverId: ownerId, productId })
            .getOne();
    }
    ...
    async findChatsByUser(userId: string): Promise<{ chats: Chat[] }> {
        ...
        for (const chat of chats) {
            const latestMessage = await this.chatsRepository.createQueryBuilder('chat')
            ...
            chat['latestMessage'] = latestMessage?.messages && latestMessage.messages[0] && latestMessage.messages[0].sender
                ? {
            ...
            if (chat.owner) {
                chat.participants = [
                    chat.participants.find(participant => participant.id === chat.owner.id),
                    ...chat.participants.filter(participant => participant.id !== chat.owner.id),
                ];
            }
    ...
    async findAllChats(limit: number, page: number): Promise<{ chats: ChatSummaryDto[], totalCount: number }> {
        ...
        const chatSummaries = chats.map((chat) => {
            const lastMessage = chat.messages.length
                ? chat.messages.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime())[0]
                : null;
            ...
            return {
                ...
                lastMessageDate: lastMessage ? lastMessage.timestamp.toISOString() : null,
