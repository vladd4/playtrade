...
export class WebSocketBotGateway {
    ...
    async handleCloseChat(@MessageBody() data: { senderId: string }) {
        ...
        await this.bot.telegram.sendMessage(user.telegramId, 'Чат з ТП завершено. Натисніть /start щоб далі користуватись ботом'); // Используйте инъецированный бот
    ...
    async handleMessage(@MessageBody() data: { senderId: string; content: string; adminId: string }) {
        try {
            ...
            const message = await this.supportChatService.createOrUpdateChatWithLastMessage(user.id, content, adminId);
    ...
    async handleSendPhoto(@MessageBody() data: { senderId: string; photoUrl: string; adminId: string }) {
        try {
            ...
            const message = await this.supportChatService.createOrUpdateChatWithLastMessage(user.id, photoUrl, adminId);
            ...
            throw new BadRequestException('Произошла ошибка при отправке фотографии.');
        }
    }
}
