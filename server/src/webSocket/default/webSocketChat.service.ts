...
export class WebSocketChatService {
  ...
  async handleChatCreation(createMessageDto: CreateMessageDto) {
    const { senderId, receiverId, productId, content, timestamp } =
      createMessageDto;
    ...
    let chat = await this.chatsService.findChatBetweenUsers(
      senderId,
      receiverId,
      productId,
    );
    ...
    const existingMessages = await this.messageQueryService.findMessagesByChat(
      chat.id,
    );
    const duplicateMessage = existingMessages.find(
      (msg) => msg.content === content && msg.timestamp === timestamp,
    );
