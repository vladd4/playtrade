...
export async function getProducts(page = 1): Promise<AdminProductsWithPagination | null> {
  try {
...
export async function getFilteredProductsByGameId(params: FilterParams): Promise<{
...
export async function getMyProductsByUserId(userId: string): Promise<Product[] | null> {
  try {
    const { data } = await privateAxios.get<Product[]>(`/users/${userId}/products`);
    return data;
  } catch (error: any) {
    console.log(
...
export async function updateProduct(
  ...
  try {
    const { data } = await privateAxios.put<Product>(`/products/${id}`, updatedProduct);
    return data;
  } catch (error: any) {
    console.log(
      `Error updating product with id ${id}, ${JSON.stringify(updatedProduct)}:`,
      error.response?.data || error.message,
    );
    return null;
  }
...
export async function deleteProductImage(productId: string, imageIndex: number) {
  try {
